...
class DQNAgent:
    def __init__(self, state_shape, action_size, config, device):
        ...
        self.steps_done = 0          # env steps (used for epsilon)
        self.learn_steps = 0         # gradient steps (used for target sync)
    ...
    def replay(self):
        ...
        # Q(s, a) using current (policy) net
        q_values = self.policy_net(states).gather(1, actions)

        # --- Double DQN target ---
        with torch.no_grad():
            # 1) action selection with policy_net
            next_actions = self.policy_net(next_states).argmax(dim=1, keepdim=True)
            # 2) action evaluation with target_net
        ...
        # Periodically sync target network (uses gradient step count)
        self.learn_steps += 1
